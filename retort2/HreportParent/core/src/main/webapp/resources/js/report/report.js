...
function getHeader()
{
	...
	var header={};
	$.ajax({
        url:  "/core/rep/reportHeader/getById",
        type: 'POST',
        data: {
        	"id":id
        },
...
function showFormElements(data)
{
	...
	$("#formElements").append($br);
	$br='<input class="btn btn-success" onclick="exportReport();" type="button"  value="导出" />';
	$("#formElements").append($br);
	$br="<br/><br/>";
	$("#formElements").append($br);
...
//下拉单选
function showDropLists(param,params)
{
	var dataSource=[];
	var header=getHeader();
...
function showCheckboxs(param)
{
    ...
    {
    	$str=dataSource[i].name+':<input type="checkbox" name='+param.paramsName+' value="'+dataSource[i].id+'"/>';
    	$strs.push($str);
    }
	 return $strs;
}
...
function addValueToLov(paramsName)
{
	...
	var vsetValues = grid.dataItem(grid.select());
	if(vsetValues==null)
	{
		alert(selectOne);
		return false;
...
//获取报表内容
function getReportDataSource(header,sql){
